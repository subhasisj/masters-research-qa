...
model_name = "subhasisj/de-kd-XLM-minilmv2-4"
...
def run_evaluation(batch_size, technique, trainer):
    print("Evaluating questions in KORQuaD")
    xquad = load_dataset("KETI-AIR/korquad","v1.0")

    validation_features = xquad.map(
        prepare_validation_features,
        batch_size=batch_size,
        batched=True,
        remove_columns=xquad["dev"].column_names,
    )

    raw_predictions = trainer.predict(validation_features["dev"])

    validation_features["dev"].set_format(
        type=validation_features["dev"].format["type"],
        columns=list(validation_features["dev"].features.keys()),
    )

    examples = xquad["dev"]
    features = validation_features["dev"]
    ...
    references = [
        {"id": ex["id"], "answers": ex["answers"]} for ex in xquad["dev"]
    ]
    ...
    evaluation_output_path = (
        f"./{technique}/evaluation_output_{technique}.txt"
    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"{technique}"):
        os.mkdir(f"{technique}")
    run_evaluation(batch_size, technique, trainer)
